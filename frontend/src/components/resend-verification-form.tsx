...
import { resendVerificationSchema, ResendVerificationFormData } from "@/types/validation.types";
...
export const ResendVerificationForm = memo<ResendVerificationFormProps>(({ className }) => {
  const [isSuccess, setIsSuccess] = useState(false);
  const isSubmittingRef = useRef(false);

  const { register, handleSubmit, formState, watch } = useForm<ResendVerificationFormData>({
  ...
  const onSubmit = useCallback(
    async (data: ResendVerificationFormData) => {
      ...
      try {
        ...
        toast.success("Verification email sent!", {
          description: "If an account with that email exists, we've sent a new verification link.",
        });
      } catch (error: any) {
